...
the workflow to be shown. Typically, this is done based on the BPMN process ID,
...
provided by the Business Cockpit (e.g. loading all user task for
...
users are only allowed to see some workflows
according to their roles. That means 
users will see a different set of columns.
...
use case the workflow represents, renders the user task form
...
Typically, the number of columns is limited because
users are only allowed to see some user tasks
according to their roles. That means
users will see a different set of columns.
...
proxy for all workflow modules forwarding request to
the use case's (micro-)service. A workflow module has to
configure the internal URI where it's (micro-)services
is available and the URL of the Business Cockpit's API (bpms):
...
vanillabp:
  ...
  workflow-modules:
    loan-approval:
      cockpit:
        workflow-module-uri: http://localhost:8080/loan-approval
```

On starting the (micro-)service the Business Cockpit adapter
will register all workflow modules by their URIs to
the Business Cockpit. This causes the VanillaBP Business Cockpit
to initialize the proxy exposed at the path `/wm/` concatenated 
with the workflow module's ID (e.g. `/wm/loan-approval`). The 
proxy endpoint can be used by the UI components to
...
Business Cockpit and the DevShell web application. Technically,
this is achieved by using [a proxy](./src/main/webapp/craco.config.js).
