...
public class JwtMapper
        extends io.vanillabp.cockpit.commons.security.jwt.JwtMapper<JwtAuthenticationToken> {
    ...
    protected JwtAuthenticationToken buildAuthenticationToken(
            final Jwt jwt, final Collection<GrantedAuthority> bcAuthorities) {
    ...
    private static void addAuthorities(
            Collection<GrantedAuthority> bcAuthorities, List<String> authorities) {
        if (authorities != null) {
    ...
    protected void applyJwtClaimsSet(
            final JwtClaimsSet.Builder claimsSetBuilder, final SecurityContext context) {
